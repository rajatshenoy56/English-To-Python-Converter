...
def evaluate(sentence, max_length=60):
    ...
    text = tokenizers.py.detokenize(output)[0] 

    tokens = tokenizers.py.lookup(output)[0]
    return text, tokens, attention_weights

def print_translation(sentence, tokens, ground_truth):
    return tokens.numpy().decode("utf-8")
...
def text(request):
    ...
    if request.method == "POST":
        ...
        print(sentence[1:-1])
        print(sentence)
        if detect_en(sentence):
            ground_truth = ""
            translated_text, translated_tokens, attention_weights = evaluate(
                sentence[1:-1])
            output = print_translation(sentence, translated_text, ground_truth)
            return JsonResponse({'lang': "English", "output": output})
        else:
...
def file(request):
    if request.method == "POST":
        ...
        for chunk in request.FILES['File'].chunks():
            str_file += str(chunk.decode('utf-8'))
        print(str_file)
        contents = str_file.split("\n")
        print(contents)
        output = ""
        ...
        # if detect_en(str_file) == True:
            if content != "":
                ...
                if output!="" and output[-1] == ':':
                    output += "\n" + "\t" +print_translation(content, translated_text, ground_truth)
                else:
                    output += "\n" +print_translation(content, translated_text, ground_truth) 
